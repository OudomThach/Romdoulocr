...
export async function copyToClipboard(text: string): Promise<boolean> {
  // Modern async Clipboard API — only available in a SECURE context (https or
  // localhost). Over plain HTTP on a LAN IP / hostname it's undefined, so we
  // fall through to the legacy path below instead of failing outright.
  try {
  ...
  // Legacy fallback: a hidden textarea + execCommand('copy'). Deprecated but
  // works over plain HTTP and in older browsers where the async API is blocked.
  try {
    ...
    // ignore — fall to the error toast below
  }
