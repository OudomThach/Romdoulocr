...
export function downloadPageXlsx(page: PageResult, filenameBase: string): void {
  ...
  const bytes = new TextEncoder().encode(xls);
...
export function downloadPageCsvs(page: PageResult, filenameBase: string): void {
  ...
  if (csvs.length === 1) {
    downloadText(`${filenameBase}-${csvs[0].name}`, csvs[0].text, 'text/csv;charset=utf-8');
    return;
  }
  for (const c of csvs) {
    downloadText(`${filenameBase}-${c.name}`, c.text, 'text/csv;charset=utf-8');
  }
}
