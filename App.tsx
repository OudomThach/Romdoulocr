...
interface TabDef {
  id: 'document' | 'translated' | 'table' | 'ocr' | 'history';
  label: string;
      ...
      <header className="sticky top-0 z-20 border-b border-slate-200 bg-white/90 backdrop-blur">
        <div className="mx-auto flex max-w-[1600px] items-center justify-between px-4 py-3 sm:px-6">
          <div className="flex items-center gap-2.5">
            <div className="grid h-8 w-8 place-items-center rounded-lg bg-slate-950 text-sm font-bold text-white">
              K
            </div>
            <h1 className="text-sm font-semibold tracking-tight text-slate-950">Khmer Document Parser</h1>
          </div>
          <div className="flex items-center gap-2">
            <button
              type="button"
              onClick={() => toggleMuted()}
              className="grid h-9 w-9 place-items-center rounded-lg border border-slate-200 bg-white text-slate-600 shadow-sm transition-colors hover:bg-slate-50 hover:text-slate-950"
          ...
          <div className="flex gap-1 overflow-x-auto">
            {TABS.map((t) => (
              <button
                ...
                className={`flex shrink-0 items-center border-b-2 px-3 py-2.5 text-sm font-medium transition-colors ${
                  activeTabForUi === t.id
